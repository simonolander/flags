...
policy_json=$(aws iam create-policy "--policy-name=${policy_name}" --policy-document '{ "Version": "2012-10-17", "Statement": [ { "Effect": "Allow", "Action": [ "s3:PutObject", "s3:GetObject" ], "Resource": "arn:aws:s3:::'"${bucket_name}"'" } ] }')
